...
const StatisticsTab: React.FC<StatisticsTabProps> = ({ userRole }) => {
  ...
  const fetchStatistics = async () => {
    ...
    try {
      ...
      if (selectedYear !== 'all' || dateRange.start || dateRange.end) {
      ...
      const monthlyData = (dateRange.start || dateRange.end) ? sortedMonthlyData : sortedMonthlyData.slice(-6);
      ...
      // Fetch feedback statistics with date filtering
      let feedbackQuery = supabase
        .from('feedback')
        .select(`
          *,
          thesis_submissions!inner(thesis_title, submission_date)
        `)
        .order('created_at', { ascending: false });

      // Apply the same date filtering to feedback based on associated thesis submission dates
      if (selectedYear !== 'all') {
        const startOfYear = `${selectedYear}-01-01`;
        const endOfYear = `${selectedYear}-12-31`;
        ...
        feedbackQuery = feedbackQuery
          .gte('thesis_submissions.submission_date', dateRange.start)
          .lte('thesis_submissions.submission_date', dateRange.end);
      } else if (dateRange.start) {
        feedbackQuery = feedbackQuery
          .gte('thesis_submissions.submission_date', dateRange.start);
      } else if (dateRange.end) {
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  {stats.previousPeriod.lpuStudents > 0 && (() => {
                    ...
                    const showPercentage = Math.abs(percentChange) <= 200;
