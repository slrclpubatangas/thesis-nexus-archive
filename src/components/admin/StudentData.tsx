...
const StudentData: React.FC = () => {
    ...
    try {
      await mutateWithAuth(
        supabase.from('students').insert({
          ...
          course_section: newStudent.course_section,
        ...
        }),
        {
          onError: (err) => {
            ...
            toast({ title: 'Error', description: 'Failed to add student record.', variant: 'destructive' });
            throw err; // Re-throw to let modal handle loading state
          },
        }
      );

      toast({ title: 'Success', description: 'Student record added successfully.' });
      queryClient.invalidateQueries({ queryKey: ['student-data'] });
      setIsAddOpen(false);
    } catch (e) {
      console.error('Add student operation failed:', e);
      throw e; // Re-throw for modal error handling
    }
  };
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const allIds = filtered.map(student => student.student_no);
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmDelete = window.confirm(
      `Are you sure you want to delete ${selectedStudentIds.length} student record(s)? This action cannot be undone.`
    );
    ...
    try {
      await mutateWithAuth(
          ...
          .in('student_no', selectedStudentIds),
        {
          onError: (error) => {
            ...
            toast({
              title: "Error",
              description: "Failed to delete student records.",
              variant: "destructive",
            });
          },
        }
      ...
      toast({
        title: "Success",
        description: `Successfully deleted ${selectedStudentIds.length} student record(s).`,
      });
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex space-x-3">
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeView === 'manage' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
          >
            Manage Data
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeView === 'upload' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
          >
            CSV Upload
          </button>
