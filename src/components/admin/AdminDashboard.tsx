...
const AdminDashboard = () => {
  ...
  const { user, loading: authLoading, shouldShowRefreshBanner, dismissRefreshBanner } = useAuth();
  ...
  useEffect(() => {
    const fetchUserRole = async () => {
      ...
      try {
        ...
        if (!data) {
          // User not found in system_users table - check if they have a profile with viewer role
          ...
          // Default to Viewer role if user exists in auth but not in system_users
          const defaultRole = profileData?.role === 'admin' ? 'Admin' : 'Viewer';
          console.log('Setting default role:', defaultRole);
          setUserRole(defaultRole);
        } else {
          console.log('User role found:', data.role);
          // Map database 'Reader' role to 'Viewer' for component interface
          const mappedRole = data.role === 'Reader' ? 'Viewer' : data.role as 'Admin' | 'Viewer';
          setUserRole(mappedRole);
        }
      } catch (error) {
        console.error('Error fetching user role:', error);
        // Default to Viewer role on any error to allow basic access
        setUserRole('Viewer');
      } finally {
  ...
  // Show refresh indicator when returning to tab while loading
  useEffect(() => {
    if (justBecameVisible && (authLoading || loading || roleLoading)) {
      toast({
        title: "Refreshing",
        description: "Refreshing page data...",
      });
    }
  }, [justBecameVisible, authLoading, loading, roleLoading, toast]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Refresh Banner */}
      <RefreshBanner
        show={shouldShowRefreshBanner}
        onDismiss={dismissRefreshBanner}
      ...
      <div className="flex-1 lg:ml-0 min-h-screen">
        <div className={`${activeTab === 'records' || activeTab === 'thesis' || activeTab === 'students' ? 'max-w-[1600px]' : 'max-w-7xl'} mx-auto px-4 lg:px-8 py-8 mt-16 lg:mt-0`}>
          {/* Tab Content */}
