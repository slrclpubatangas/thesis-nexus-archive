    ...
    <>
      ...
      >
        <div className="w-6 h-5 flex flex-col justify-between">
          <span
            className={`block h-0.5 w-full bg-gray-800 rounded-full transition-all duration-300 ease-in-out ${
      ...
      >
        <ClickSpark sparkColor="#dc2626" sparkSize={8} sparkRadius={12} sparkCount={6} duration={350}>
          ...
          {/* Sidebar Header - Only show if Reader role and not collapsed */}
          {userRole === 'Reader' && !isCollapsed && (
          ...
          {/* Reader badge always visible on mobile */}
          {userRole === 'Reader' && (
          ...
          <div className="p-4 border-t border-gray-200">
            <div className={`text-xs text-gray-500 transition-all duration-300 ${
              isCollapsed ? 'lg:text-center' : 'text-center'
            }`}>
              {!isCollapsed && (
                <p className="opacity-0 animate-fadeIn" style={{ animationDelay: '300ms', animationFillMode: 'forwards' }}>Admin Dashboard</p>
              )}
              <p className={`${
                isCollapsed ? 'lg:mt-0' : 'mt-1'
              }`}>© 2025{isCollapsed ? '' : <span className="opacity-0 animate-fadeIn" style={{ animationDelay: '300ms', animationFillMode: 'forwards' }}> UndergradFile</span>}</p>
            </div>
          </div>
          </div>
        </ClickSpark>
