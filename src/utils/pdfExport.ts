...
export const exportStatisticsToPDF = async (
  ...
  try {
    ...
    checkPageSpace(30 + Math.min(stats.popularPrograms.length, 10) * 7);
    pdf.setFontSize(16);
    pdf.setFont('helvetica', 'bold');
    ...
    stats.popularPrograms.slice(0, 10).forEach((program, index) => {
      checkPageSpace(7);
      const text = `${index + 1}. ${program.name}: ${program.count} submissions (${program.percentage}%)`;
    ...
    // Add Student Type Distribution Chart
    if (studentTypeChart) {
      checkPageSpace(80);
      pdf.setFontSize(16);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Student Type Distribution', 20, yPosition);
      yPosition += 10;
    ...
    if (campusDistChart) {
      ...
      const imgWidth = pageWidth - 40;
      const imgHeight = 110;
      pdf.addImage(campusDistChart, 'PNG', 20, yPosition, imgWidth, imgHeight);
      yPosition += imgHeight + 10;
    }
    ...
    if (monthlyTrendChart) {
      ...
      const imgWidth = pageWidth - 40;
      const imgHeight = 65;
      pdf.addImage(monthlyTrendChart, 'PNG', 20, yPosition, imgWidth, imgHeight);
      yPosition += imgHeight + 10;
    }
    ...
    // Add Feedback Distribution Chart
    if (feedbackDistChart) {
      checkPageSpace(80);
      pdf.setFontSize(14);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Feedback Distribution', 25, yPosition);
      yPosition += 5;

      const imgWidth = pageWidth - 40;
      const imgHeight = 70;
      pdf.addImage(feedbackDistChart, 'PNG', 20, yPosition, imgWidth, imgHeight);
      yPosition += imgHeight + 10;
    } else {
      checkPageSpace(50);
      pdf.setFontSize(14);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Feedback Distribution (Text Summary)', 25, yPosition);
      yPosition += 7;
